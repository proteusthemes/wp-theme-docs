...
function sassRoots(theme) {
  return [path.join(SRC, theme, "sass"), path.join(SRC, "master/sass")];
}
...
function buildJs(theme) {
  const sources = [path.join(SRC, "master/scripts/main.js")];
  ...
  return writeAsset(path.join(BUILD, theme, "js"), "main", "js", bundle);
}
